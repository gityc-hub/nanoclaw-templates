...
requests, selected book lists, Tech lead source briefs, and per-skill expert
outputs. The shared
`references/agents/expert-protocol.md` defines book-guided expert exchanges;
the Tech lead uses its own source brief. Only the coordinator writes
user-facing prose. The per-skill `evals/evals.json` files are
...
the current-source trace separately.
